...
function addToCart(e) {
    ...
    qtyValue.setAttribute('id', 'inputqty');
    ...
    document.getElementById('itemDetails').addEventListener('click', btnQty);
...
function btnQty(e) {
    ...
    let strPrice = document.getElementById('price').innerText;

    // gets only numbers from string
    const price = strPrice.replace(/^\D+/g, "");

    // add or del
    let btnQty = e.target.childNodes["0"].data;
    // value
    ...
    } else if (btnQty === 'rem' && inputQtyNum >= 1) {
        inputQty.value = parseInt(inputQty.value) - 1;
        inputQty.innerText = inputQty.value;
    }

    total = price * inputQty.value;
    totalCost.innerText = 'Total: ' + total;
