...
$(document).ready(function() {
  ...
  // initializes object
  shoppingCart.initialize(shoppingCartElements);

  $(".js-fruitImage").click(function() {
    let fruitId = parseInt($(this).attr("fruit_id"));

    // if product not exist in cart, adds product to cart
    if(!shoppingCart.hasFruit(fruitsObject[fruitId])) {
      ...
      let shoppingCartItemTemplate = _.template(
        ...
        "<input class='js-price-input<%= fruitId %>'>" +
      ...
      $(".js-remove").click(function() {
        ...
        $(".js-price-input"+fruitId).val(shoppingCart.getFruitQuantity(fruitsObject[parseInt($(this).parent().attr("fruit_id"))]));
      });
      ...
      $(".js-price-input"+fruitId).val(shoppingCart.getFruitQuantity(fruitsObject[parseInt($(".js-price-input"+fruitId).parent().attr("fruit_id"))]));
    }
    // if product exist in cart, only updates price
    else if(shoppingCart.hasFruit(fruitsObject[fruitId])) {
      shoppingCart.addFruit(fruitsObject[fruitId], 1);

      
      $(".js-price-input"+fruitId).val(shoppingCart.getFruitQuantity(fruitsObject[parseInt($(".js-price-input"+fruitId).parent().attr("fruit_id"))]));
console.log($(".js-price-input"+fruitId));
